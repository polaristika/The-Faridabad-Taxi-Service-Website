...
const Navbar: React.FC<NavbarProps> = ({ config }) => {
  ...
  // Admin removed from top nav as requested
  const navLinks = [
    { name: 'Home', path: '/' },
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a
              ...
              className="bg-blue-600 text-white px-5 py-2 rounded-full text-sm font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all flex items-center gap-2"
            >
        ...
        <div className="md:hidden bg-white border-t border-slate-100 p-4 space-y-4 shadow-xl">
          {navLinks.map((link) => (
            <button
              key={link.name}
              onClick={() => handleNavClick(link.path)}
              className="block w-full text-left text-slate-600 font-medium py-2 px-4 hover:bg-slate-50 rounded-lg"
            >
              {link.name}
            </button>
          ))}
